...
flags.DEFINE_string('rank_rate_svd', '3/4', "rank rate tt")
...
flags.DEFINE_string('warm_up_epoch','10','how many fine-tune epoch when warm up is used')

flags.DEFINE_string('ckpt_path', '20201009', 'ckpt path to restore' )
...
def decompose(weight, rank):
    ...
    assert(rank==int(O*eval(FLAGS.rank_rate_svd)))
...
def basic_conv(in_tensor, value_dict, layer_num, repeat_num, conv_num, flag_shortcut, down_sample, is_training, res_in_tensor=None, rank_rate=None):
    ...
    #each first layer of each compressed block is decompoed with SVD
    elif repeat_num==0 and conv_num==1: 
    ...
    #Other layers are compressed with proposed NC-CSVD
    else:
        name_scope = 'layer'+ str(layer_num) + '.'+str(repeat_num) + '.'
        name_weight = name_scope + 'conv' + str(conv_num) + '.weight'
        name_bn_scope = name_scope + 'bn' + str(conv_num)

        independ_component = basic_conv_independent(in_tensor, layer_num, repeat_num, conv_num,strides[1])
        shared_component = basic_conv_partly_shared(in_tensor, layer_num, repeat_num, conv_num,strides[1])
...
var_list_to_train = []  #fine-tune liset
rank_rate_svd = eval(FLAGS.rank_rate_svd)
...
rc = eval(FLAGS.rc)
ri = eval(FLAGS.ri)
proposed_dict = np.load('/home/test01/sambashare/sdd/ICASSP/resnet34/Decom/Widen/No_Relu/Iter/resnet34_csvd_npy/iter%d_npy_rc=%f_ri=%f.npy'%(eval(FLAGS.iter),round(rc,4),round(ri,6)), allow_pickle=True).item()
for item in proposed_dict:
    print(item)


ckpt_root_path = ckpt_path+'iter%d'%eval(FLAGS.iter)+ '/rc=%f_ri=%f'%(round(rc,4),round(ri,6)) +'_r_svd='+FLAGS.rank_rate_svd
os.makedirs(ckpt_root_path, exist_ok=True)
log_path = ckpt_root_path+ '/rc=%f_ri=%f'%(round(rc,4),round(ri,6)) +'_r_svd='+FLAGS.rank_rate_svd + '.log'
...
tensor_go_next_fc, output = resnet34(x, value_dict, is_training, rank_rate_svd)
...
with tf.Session(config = config) as sess:
    ...
    try:
        ...
        with open(log_path, 'a') as f:
            f.write('rank_rate_svd: ' + FLAGS.rank_rate_svd)
        ...
        for j in range(epoch):
            ...
            for i in range(int(n/128)):
                ...
                '''For rigorism, is_trianing has to be False to keep parameters of BN layer the same'''
            ...
            with open(log_path, 'a') as f:
                if j==0:
                    f.write('rank_rate:'+str(FLAGS.rank_rate_svd)+'\n')
                ...
                if j+1==epoch:
                    ...
                    print(ave_time)
                    saver.save(sess, ckpt_root_path+'/resnet34csvd-acc'+str(ave_time)[0:7]+'-ave3time'+str(ave_time)[0:7], global_step=j+1)
                else:
                    saver.save(sess, ckpt_root_path+'/resnet34csvd-acc'+str(test_acc)[0:7]+'-time'+str(time_ave_one_epoch)[0:7], global_step=j+1)
